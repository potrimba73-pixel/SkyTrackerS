...
# HTML Dashboard inline - COMPLETO COM TODAS AS FUNCIONALIDADES
    ...
    <div class="detail-section">
        ...
        <div class="data-grid">
            <div class="data-item">
                ...
                <div class="value" id="detail-lat">---</div>
            </div>
            <div class="data-item">
                <div class="label">Longitude</div>
                <div class="value" id="detail-lon">---</div>
            </div>
            <div class="data-item">
                <div class="label">Altitude (Baro)</div>
                <div class="value highlight" id="detail-alt">---</div>
            </div>
            <div class="data-item">
                <div class="label">Altitude (GPS)</div>
                <div class="value" id="detail-alt-gps">---</div>
            </div>
            <div class="data-item">
                <div class="label">Velocidade Solo</div>
                <div class="value highlight" id="detail-gs">---</div>
            </div>
            <div class="data-item">
                <div class="label">Velocidade Indicada</div>
                <div class="value" id="detail-ias">---</div>
            </div>
            <div class="data-item">
                <div class="label">Razao Vertical</div>
                <div class="value" id="detail-vsi">---</div>
            </div>
            <div class="data-item">
                <div class="label">Rumo (Track)</div>
                <div class="value" id="detail-track">---</div>
            </div>
        </div>
    </div>
    ...
    <div class="detail-section">
        ...
        <div class="data-grid">
            <div class="data-item">
                ...
                <div class="value highlight" id="detail-squawk">---</div>
            </div>
            <div class="data-item">
                <div class="label">Numero Mach</div>
                <div class="value" id="detail-mach">---</div>
            </div>
            <div class="data-item">
                <div class="label">Temperatura</div>
                <div class="value" id="detail-temp">---</div>
            </div>
            <div class="data-item">
                <div class="label">Vento</div>
                <div class="value" id="detail-wind">---</div>
            </div>
            <div class="data-item">
                <div class="label">Pressao</div>
                <div class="value" id="detail-pressure">---</div>
            </div>
            <div class="data-item">
                <div class="label">No Solo</div>
                <div class="value" id="detail-ground">---</div>
            </div>
        </div>
    </div>
    ...
    <div class="detail-section">
        ...
        <div class="data-grid">
            <div class="data-item">
                ...
                <div class="value highlight" id="detail-type">---</div>
            </div>
            <div class="data-item">
                <div class="label">Matricula</div>
                <div class="value" id="detail-reg">---</div>
            </div>
            <div class="data-item">
                <div class="label">Fabricante</div>
                <div class="value" id="detail-manuf">---</div>
            </div>
            <div class="data-item">
                <div class="label">Modelo</div>
                <div class="value" id="detail-model">---</div>
            </div>
            <div class="data-item">
                <div class="label">Envergadura</div>
                <div class="value" id="detail-wingspan">---</div>
            </div>
            <div class="data-item">
                <div class="label">Passageiros</div>
                <div class="value" id="detail-pax">---</div>
            </div>
        </div>
    </div>
    ...
    <div class="detail-section">
        ...
        <div class="data-grid">
            <div class="data-item">
                ...
                <div class="value highlight" id="detail-airport-name">---</div>
            </div>
            <div class="data-item">
                <div class="label">Distancia</div>
                <div class="value" id="detail-airport-dist">---</div>
            </div>
            <div class="data-item">
                <div class="label">Cidade</div>
                <div class="value" id="detail-airport-city">---</div>
            </div>
            <div class="data-item">
                <div class="label">Pais</div>
                <div class="value" id="detail-airport-country">---</div>
            </div>
        </div>
    </div>
...
function getAirlineInfo(callsign){
    if(!callsign) return {name:"Desconhecida",country:"Desconhecido",flag:"&#9992;&#65039;"};
    const cs=callsign.toUpperCase();
    for(let prefix of Object.keys(AIRLINES).sort((a,b)=>b.length-a.length)){
        if(cs.startsWith(prefix)) return AIRLINES[prefix]
    }
    return {name:"Desconhecida",country:"Desconhecido",flag:"&#9992;&#65039;"}
}
...
function createAircraftIcon(heading,color,callsign){
    const airline=getAirlineInfo(callsign);
    const svg='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" width="36" height="36"><g transform="rotate('+(heading||0)+',50,50)"><path d="M50 10 L20 50 L30 55 L50 45 L70 55 L80 50 Z" fill="'+color+'" stroke="white" stroke-width="1.5"/><ellipse cx="50" cy="50" rx="6" ry="35" fill="'+color+'" stroke="white" stroke-width="1.5"/><path d="M50 80 L35 95 L50 90 L65 95 Z" fill="'+color+'" stroke="white" stroke-width="1.5"/><ellipse cx="50" cy="22" rx="4" ry="6" fill="#1e293b" stroke="white" stroke-width="0.5"/><line x1="50" y1="30" x2="50" y2="70" stroke="white" stroke-width="0.8" stroke-dasharray="3,3"/></g></svg>';
    return L.divIcon({html:svg,className:'aircraft-icon',iconSize:[36,36],iconAnchor:[18,18],popupAnchor:[0,-18]})
}
...
async function loadRegions(){
    try{
        ...
        regions.forEach(r=>{
            L.circle([r.lat,r.lon],{radius:r.radius_km*1000,color:r.color,fillColor:r.color,fillOpacity:.08,weight:2}).addTo(map).bindPopup('<b>'+r.name+'</b><br>Raio: '+r.radius_km+'km');
            L.marker([r.lat,r.lon],{icon:L.divIcon({html:'<div style="background:'+r.color+';color:white;padding:3px 8px;border-radius:4px;font-size:11px;font-weight:600;">'+r.name+'</div>',className:'region-label'})}).addTo(map)
        })
    }catch(e){console.error('Erro regioes:',e)}
}
...
async function loadFlights(){
    ...
    }catch(e){console.error('Erro voos:',e)}
}

function updateFlights(newFlights){
    flights=newFlights;
    document.getElementById('flight-count').textContent=flights.length+' voos';
    document.getElementById('flights-loading').style.display='none';
    const currentIds=new Set(flights.map(f=>f.icao24));
    Object.keys(markers).forEach(id=>{if(!currentIds.has(id)){map.removeLayer(markers[id]);delete markers[id]}});

    flights.forEach(f=>{
        const airline=getAirlineInfo(f.callsign);
        const region=regions.find(r=>r.name===f.region)||{color:'#38bdf8'};
        if(markers[f.icao24]){
            markers[f.icao24].setLatLng([f.latitude,f.longitude]);
            markers[f.icao24].setIcon(createAircraftIcon(f.heading,region.color,f.callsign))
        }else{
            const marker=L.marker([f.latitude,f.longitude],{icon:createAircraftIcon(f.heading,region.color,f.callsign)}).addTo(map);
            marker.on('click',()=>openDetailPanel(f));
            const flightTime=f.last_contact?Math.round((Date.now()/1000-f.last_contact)/60):'?';
            const popupContent=buildPopupContent(f,airline,flightTime);
            marker.bindPopup(popupContent);
            markers[f.icao24]=marker
        }
    });
    renderFlightList();
    if(selectedFlight) updateDetailPanel(selectedFlight)
}

function buildPopupContent(f,airline,flightTime){
    return '<div style="min-width:220px;font-family:system-ui">'+
        '<div style="display:flex;align-items:center;gap:8px;border-bottom:1px solid #334155;padding-bottom:8px;margin-bottom:8px">'+
        '<span style="font-size:1.4rem">'+airline.flag+'</span>'+
        '<div><div style="font-size:1.1rem;font-weight:700">'+(f.callsign||'N/A')+'</div>'+
        '<div style="font-size:.75rem;color:#94a3b8">'+airline.name+'</div></div></div>'+
        '<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:.78rem">'+
        '<div><i class="fas fa-arrows-alt-v" style="color:#38bdf8"></i> '+Math.round(f.altitude||0).toLocaleString()+' ft</div>'+
        '<div><i class="fas fa-tachometer-alt" style="color:#38bdf8"></i> '+Math.round((f.velocity||0)*3.6)+' km/h</div>'+
        '<div><i class="fas fa-compass" style="color:#38bdf8"></i> '+Math.round(f.heading||0)+'°</div>'+
        '<div><i class="fas fa-clock" style="color:#38bdf8"></i> '+flightTime+' min</div>'+
        '</div>'+
        '<div style="margin-top:8px;padding-top:8px;border-top:1px solid #334155;font-size:.75rem;color:#94a3b8">'+
        '<div><strong style="color:#e2e8f0">&#127759; Regiao:</strong> '+(f.region||'N/A')+'</div>'+
        '<div><strong style="color:#e2e8f0">&#128226; Squawk:</strong> '+(f.squawk||'N/A')+'</div>'+
        '</div></div>'
}

function renderFlightList(){
    const container=document.getElementById('flights-list');
    if(!flights.length){container.innerHTML='<p style="color:#94a3b8;text-align:center;padding:20px">Nenhum voo ativo</p>';return}
    container.innerHTML=flights.map(f=>{
        const airline=getAirlineInfo(f.callsign);
        const region=regions.find(r=>r.name===f.region)||{color:'#38bdf8'};
        const flightTime=f.last_contact?Math.round((Date.now()/1000-f.last_contact)/60):'?';
        return '<div class="flight-card" onclick="openDetailPanel(flights.find(x=>x.icao24===\''+f.icao24+'\'))">'+
            '<div class="airline"><span class="airline-flag">'+airline.flag+'</span><div><div class="callsign">'+(f.callsign||'N/A')+'</div><div class="airline-name">'+airline.name+'</div></div></div>'+
            '<div class="details">'+
            '<div class="detail-item"><i class="fas fa-arrows-alt-v"></i> '+Math.round(f.altitude||0).toLocaleString()+' ft</div>'+
            '<div class="detail-item"><i class="fas fa-tachometer-alt"></i> '+Math.round((f.velocity||0)*3.6)+' km/h</div>'+
            '<div class="detail-item"><i class="fas fa-compass"></i> '+Math.round(f.heading||0)+'°</div>'+
            '<div class="detail-item"><i class="fas fa-clock"></i> '+flightTime+' min</div>'+
            '</div>'+
            '<span class="region-tag" style="background:'+region.color+'22;color:'+region.color+'">&#127759; '+(f.region||'N/A')+' ('+(f.distance_from_center?f.distance_from_center.toFixed(1):'?')+' km)</span>'+
            '</div>'
    }).join('')
}
...
function focusFlight(icao24){
    ...
    if(f){map.setView([f.latitude,f.longitude],13);if(markers[icao24])markers[icao24].openPopup()}
}

// ============ DETAIL PANEL ============
function openDetailPanel(flight){
    if(!flight) return;
    selectedFlight = flight;
    updateDetailPanel(flight);
...
function updateDetailPanel(f){
    ...
    // Header
    document.getElementById('detail-callsign').textContent = f.callsign || 'N/A';
    document.getElementById('detail-flag').innerHTML = airline.flag;
    document.getElementById('detail-airline').textContent = airline.name + ' | ' + airline.country;

    // Route
    const route = f.route || {};
    document.getElementById('detail-origin-code').textContent = route.origin_icao || '???';
    document.getElementById('detail-origin-name').textContent = (route.origin || 'Desconhecido').substring(0, 15);
    document.getElementById('detail-dest-code').textContent = route.destination_icao || '???';
    document.getElementById('detail-dest-name').textContent = (route.destination || 'Desconhecido').substring(0, 15);

    const progress = route.progress || 0;
    document.getElementById('route-progress-bar').style.width = progress + '%';
    document.getElementById('route-plane-icon').style.left = progress + '%';
    document.getElementById('route-progress-text').textContent = 'Progresso: ' + progress + '%';
    document.getElementById('route-distance').textContent = 'Distancia: ' + (route.distance_total_km || 0) + ' km (' + (route.distance_remaining_km || 0) + ' km restantes)';

    // Position & Movement
    document.getElementById('detail-lat').textContent = f.latitude ? f.latitude.toFixed(4) : '---';
    document.getElementById('detail-lon').textContent = f.longitude ? f.longitude.toFixed(4) : '---';
    document.getElementById('detail-alt').textContent = f.altitude ? Math.round(f.altitude).toLocaleString() + ' ft' : '---';
    document.getElementById('detail-alt-gps').textContent = f.altitude_gps ? Math.round(f.altitude_gps).toLocaleString() + ' ft' : '---';
    document.getElementById('detail-gs').textContent = f.velocity ? Math.round(f.velocity * 3.6) + ' km/h' : '---';
    document.getElementById('detail-ias').textContent = f.velocity ? Math.round(f.velocity * 1.94384) + ' kts' : '---';
    document.getElementById('detail-vsi').textContent = f.vertical_rate ? Math.round(f.vertical_rate) + ' ft/min' : '---';
    document.getElementById('detail-track').textContent = f.heading ? Math.round(f.heading) + '°' : '---';

    // Flight Parameters
    document.getElementById('detail-squawk').textContent = f.squawk || '---';
    document.getElementById('detail-mach').textContent = f.mach ? 'M' + f.mach : '---';

    const weather = f.weather || {};
    document.getElementById('detail-temp').textContent = weather.temperature ? weather.temperature + '°C' : '---';
    document.getElementById('detail-wind').textContent = (weather.wind_speed && weather.wind_direction) ? weather.wind_direction + '° / ' + weather.wind_speed + ' km/h' : '---';
    document.getElementById('detail-pressure').textContent = weather.pressure ? weather.pressure + ' hPa' : '---';
    document.getElementById('detail-ground').textContent = f.on_ground ? 'Sim' : 'Nao';

    // Aircraft Info
    const acInfo = f.aircraft_info || {};
    const acSpecs = f.aircraft_specs || {};
    document.getElementById('detail-type').textContent = acInfo.typecode || '---';
    document.getElementById('detail-reg').textContent = acInfo.registration || '---';
    document.getElementById('detail-manuf').textContent = acInfo.manufacturer_icao || '---';
    document.getElementById('detail-model').textContent = acInfo.model || '---';
    document.getElementById('detail-wingspan').textContent = acSpecs.wingspan ? acSpecs.wingspan + ' m' : '---';
    document.getElementById('detail-pax').textContent = acSpecs.passengers ? acSpecs.passengers : '---';

    // Nearest Airport
    const airport = f.nearest_airport || {};
    document.getElementById('detail-airport-name').textContent = airport.name || '---';
    document.getElementById('detail-airport-dist').textContent = airport.distance_km ? airport.distance_km + ' km' : '---';
    document.getElementById('detail-airport-city').textContent = airport.city || '---';
    document.getElementById('detail-airport-country').textContent = airport.country || '---';
}
...
async function loadStats(){
    try{
        ...
        document.getElementById('stat-active').textContent=s.active_flights||0;
        document.getElementById('stat-countries').textContent=s.unique_countries||0;
        document.getElementById('stat-max-alt').textContent=s.max_altitude?Math.round(s.max_altitude).toLocaleString():0;
        renderCharts(s)
    }catch(e){console.error('Erro stats:',e)}
}

function renderCharts(s){
    if(!s||typeof s!=='object'){console.log('Stats vazias');return}
    ...
    if(!hourlyCanvas||!countriesCanvas||!altitudeCanvas){return}

    const container=hourlyCanvas.parentElement;
    if(!container||container.clientHeight===0||container.clientWidth===0){return}

    try{if(charts.hourly)charts.hourly.destroy()}catch(e){}
    try{if(charts.countries)charts.countries.destroy()}catch(e){}
    try{if(charts.altitude)charts.altitude.destroy()}catch(e){}
        ...
        try{
            charts.hourly=new Chart(hourlyCanvas,{
                ...
                data:{
                    ...
                    datasets:[{label:'Voos',data:hourly.map(h=>h.count),borderColor:'#38bdf8',backgroundColor:'rgba(56,189,248,0.1)',fill:true,tension:.4}]
                },
                options:{
    ...
    if(countries.length>0){
        try{
            charts.countries=new Chart(countriesCanvas,{
                type:'doughnut',
                data:{
                    labels:countries.map(c=>c.country),
                    datasets:[{data:countries.map(c=>c.count),backgroundColor:['#38bdf8','#22c55e','#f59e0b','#ef4444','#a855f7','#ec4899','#14b8a6','#f97316']}]
                },
                options:{
                    responsive:true,maintainAspectRatio:false,
    ...
    const altRanges=s.altitude_distribution||[];
    if(altRanges.length>0){
        try{
            charts.altitude=new Chart(altitudeCanvas,{
                type:'bar',
                data:{
                    labels:altRanges.map(a=>a.range),
                    datasets:[{label:'Avioes',data:altRanges.map(a=>a.count),backgroundColor:'#38bdf8',borderRadius:4}]
                },
                options:{
                    responsive:true,maintainAspectRatio:false,
                    plugins:{legend:{display:false}},
...
async function loadAlerts(){
    try{
        ...
        document.getElementById('alert-badge').classList.toggle('show',alerts.length>0);
        const container=document.getElementById('alerts-list');
        if(!alerts.length){container.innerHTML='<p style="color:#94a3b8;text-align:center;padding:20px">Nenhum alerta</p>';return}
        container.innerHTML=alerts.map(a=>'<div class="alert-item"><div class="time">'+new Date(a.timestamp).toLocaleString('pt-PT')+'</div><div class="message">&#128680; <strong>'+a.callsign+'</strong> detetado em '+a.region+' &mdash; '+(a.distance_km?a.distance_km.toFixed(1):'?')+' km</div></div>').join('')
...
function connectWS(){
    ...
    ws.onmessage=(e)=>{
        try{
            ...
            if(data.type==='init'||data.type==='update'){updateFlights(data.flights||[])}
